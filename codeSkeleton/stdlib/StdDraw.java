...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    // static initializer
    // static { init(); }
    // JACO: disable to prevent "flashing"
    ...
    *************************************************************************/
    ...
    public static Color getPixel(int x, int y) {
    	return Color.decode(Integer.toString(onscreenImage.getRGB(x, y)));
    }
    ...
    *************************************************************************/
    ...
    /**
     * Draw an ellipse with given semimajor and semiminor axes, centered on (x, y) rotated by angle.
     * @param x the x-coordinate of the center of the ellipse
     * @param y the y-coordinate of the center of the ellipse
     * @param semiMajorAxis is the semimajor axis of the ellipse
     * @param semiMinorAxis is the semiminor axis of the ellipse
     * @param angle the degree of rotation (in radians)
     * @throws IllegalArgumentException if either of the axes are negative
     */
    public static void filledEllipse(double x, double y, double semiMajorAxis, double semiMinorAxis, double angle) {
    	if (semiMajorAxis < 0) throw new IllegalArgumentException("ellipse semimajor axis must be nonnegative");
    	if (semiMinorAxis < 0) throw new IllegalArgumentException("ellipse semiminor axis must be nonnegative");
    	double xs = scaleX(x);
    	double ys = scaleY(y);
    	...
    	double hs = factorY(2*semiMinorAxis);
    	if (ws <= 1 && hs <= 1) pixel(x, y);
    	else offscreen.fill(AffineTransform
    			.getTranslateInstance(xs, ys)
    			.createTransformedShape(AffineTransform
    				.getRotateInstance(-angle)
    				.createTransformedShape(new Ellipse2D.Double(-ws/2, -hs/2, ws, hs))));
    	draw();
    }
    ...
    *************************************************************************/
    ...
    public static void picture(String s) {
    	...
    	if (ws < 0 || hs < 0) throw new IllegalArgumentException("image " + s + " is corrupt");
    	setCanvasSize(ws, hs);
    	offscreen.drawImage(image, 0, 0, null);
    	draw();
    }
